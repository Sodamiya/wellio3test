...
export function UploadPage({ onBack, onUpload }: UploadPageProps) {
    ...
    const startCamera = async () => {
      try {
        ...
        if (error.name === "NotAllowedError" || error.name === "PermissionDeniedError") {
          ...
          setCameraError("카메라가 이미 다른 앱에서 사용 중입니다.");
        } else {
  ...
  // 이미지를 335:400 비율로 크롭/리사이즈하는 함수
  const resizeImageToFeedRatio = (imageSrc: string): Promise<string> => {
    return new Promise((resolve, reject) => {
      const img = new Image();
      img.crossOrigin = "anonymous";
      ...
      img.onload = () => {
        ...
        const targetRatio = targetWidth / targetHeight; // 0.8375
  ...
  const handleCapture = async () => {
    if (isUploadMode) {
      ...
      // 이미지 크롭/리사이즈 (항상 적용)
      try {
        finalImage = await resizeImageToFeedRatio(finalImage);
      } catch (error) {
        console.error("이미지 크롭/리사이즈 실패:", error);
        ...
        canvas.toBlob(async (blob) => {
          if (blob) {
            ...
            reader.onloadend = async () => {
              ...
              // 비율 조정 적용
              try {
                const resizedImage = await resizeImageToFeedRatio(imageData);
                setSelectedImage(resizedImage);
              } catch (error) {
                console.error("이미지 비율 조정 실패:", error);
      ...
      <div className="relative w-full h-screen bg-white overflow-hidden">
        <div className="absolute left-0 right-0 top-0 bottom-0 pt-20 pb-[120px] flex justify-center items-center overflow-hidden">
          <div className="w-full h-full flex justify-center items-center px-4">
            {/* 피드 비율 335:400에 맞춘 컨테이너 */}
            <div className="relative w-full max-w-[335px] aspect-[335/400] bg-gray-900 rounded-2xl overflow-hidden shadow-lg">
              <video
                ref={videoRef}
                autoPlay
                playsInline
